    ...
    : QObject{parent}
{
    ...
    if(videoRenderWnd){
        ...
        mpv_set_option(mpv.get(), "wid", MPV_FORMAT_INT64, &wid);
    }else{
        ...
        // vo: null 表示禁止视频输出
        // ao: null 表示禁止音频输出
        mpv_set_option_string(mpv.get(), "vo", "null");
        mpv_set_option_string(mpv.get(), "ao", "null");
    }

    // 初始化mpv实例
    if(mpv_initialize(mpv.get()) < 0){
        LOG()<<"mpv初始化失败";
        mpv_destroy(mpv.release());
        return;
    }
    ...
    mpv_observe_property(mpv.get(), 0, "time-pos", MPV_FORMAT_INT64);
    ...
    mpv_set_wakeup_callback(mpv.get(), wakeup, this);
}
...
void MpvPlayer::handleMpvEvent(mpv_event *event)
{
    ...
    case MPV_EVENT_PROPERTY_CHANGE:
    {
        ...
        if(0 == strcmp(eventProperty->name, "time-pos")){
            ...
            mpv_get_property(mpv.get(), "demuxer-start-time", MPV_FORMAT_DOUBLE, &segmentStartTime);
            segmentStartTime--;

            // 获取当前分片内的播放时间
            double segmentCurrentTime = 0;
            mpv_get_property(mpv.get(), "time-pos", MPV_FORMAT_DOUBLE, &segmentCurrentTime);
    ...
    case MPV_EVENT_END_FILE:
    {
        ...
        if(endFile->reason == MPV_END_FILE_REASON_EOF){
            ...
            mpv_get_property(mpv.get(), "playlist-count", MPV_FORMAT_INT64, &playlistCount);
            mpv_get_property(mpv.get(), "playlist-pos", MPV_FORMAT_INT64, &playlistPos);
...
void MpvPlayer::startPlay(const QString &videoPath)
{
    ...
    mpv_command_async(mpv.get(), 0, args);
}
...
void MpvPlayer::play()
{
    ...
    int pause = 0;
    mpv_set_property_async(mpv.get(), 0, "pause", MPV_FORMAT_FLAG, &pause);
}
...
void MpvPlayer::setPlaySpeed(double speed)
{
    ...
    mpv_set_property_async(mpv.get(), 0, "speed", MPV_FORMAT_DOUBLE, &speed);
}
...
void MpvPlayer::setMute(bool isMute)
{
    ...
    int flag = isMute? 1 : 0;
    mpv_set_property_async(mpv.get(), 0, "mute", MPV_FORMAT_FLAG, &flag);
}
...
void MpvPlayer::setCurrentPlayPositon(int64_t seconds)
{
    ...
    mpv_set_property_async(mpv.get(), 0, "time-pos", MPV_FORMAT_INT64, &seconds);
}
