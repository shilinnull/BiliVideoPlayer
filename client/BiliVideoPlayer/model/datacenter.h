...
private:
    ...
    std::unique_ptr<KindAndTag> kindsAndTags;
    QString loginSessionId = "";					// 当前客户端登录到服务器会话的id
    std::unique_ptr<VideoList> videoListPtr;	// 管理首页获取的视频信息
    QHash<int64_t, QList<BarrageInfo>> barrages;    // 弹幕信息
    std::unique_ptr<UserInfo> myselfInfo;           // 保存当前用户个人信息
    std::unique_ptr<UserInfo> otherUserInfo;        // 保存其他用户个人信息
    std::unique_ptr<VideoList> userVideoList;       // 保存指定用户视频列表：我的视频列表 或 其他用户视频列表
    std::unique_ptr<VideoList> statusVideoList;     // 状态视频列表
    std::unique_ptr<AdminList> adminListPtr;        // 管理员列表
signals:
