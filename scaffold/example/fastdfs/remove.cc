...
int main(int argc, char *argv[])
{
    ...
    auto tracker_server1 = tracker_get_connection();
    std::cout << (size_t)tracker_server << "   " << (size_t)tracker_server1 << std::endl;
    std::cout << tracker_server->sock << "   " << tracker_server1->sock << std::endl;
    // 4. 进行文件的上传/下载（可以手动获取一个storage节点连接然后进行操作）
    ret = storage_delete_file1(tracker_server, nullptr, argv[1]);
    if (ret != 0) {
        std::cout << "文件删除失败: " << STRERROR(ret) << std::endl;
        return -1;
    }
